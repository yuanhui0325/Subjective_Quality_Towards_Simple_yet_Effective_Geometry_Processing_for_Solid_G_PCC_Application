...
typedef struct _SchroArithEncState SchroArithEncState;
...
void schro_arith_get_encode_state(const SchroArith *arith, SchroArithEncState *state);

void schro_arith_set_encode_state(SchroArith *arith, const SchroArithEncState *state);
...
static inline void
_schro_arith_get_encode_state(const SchroArith *arith, SchroArithEncState *state)
{
  state->range[0] = arith->range[0];
  state->range[1] = arith->range[1];
  state->cntr = arith->cntr;
  state->carry = arith->carry;

  state->first_byte = arith->first_byte;
  state->output_byte = arith->output_byte;
}

static inline void
_schro_arith_set_encode_state(SchroArith *arith, const SchroArithEncState *state)
{
  arith->range[0] = state->range[0];
  arith->range[1] = state->range[1];
  arith->cntr = state->cntr;
  arith->carry = state->carry;

  arith->first_byte = state->first_byte;
  arith->output_byte = state->output_byte;
}
